"""
LLM module: Integrates with Ollama (local) or Google Gemini for NL-to-SQL translation.
...
Supports both Ollama (local, free) and Gemini (cloud, free tier).
"""
...
# Configuration: Ollama by default, Gemini as fallback
...
def init_llm():
    ...
    if LLM_PROVIDER == "ollama":
        ...
        try:
            ...
            print(f"Ollama not available ({e}). Checking Gemini fallback...")
...
def process_query(
    ...
    if not model:
        return {
            "answer": "LLM is not configured. Please ensure Ollama is running or set GEMINI_API_KEY.",
            "sql": None,
            "results": [],
            "highlight_nodes": [],
